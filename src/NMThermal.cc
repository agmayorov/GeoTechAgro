...
NMThermal::NMThermal(G4NistManager *nistMan, G4Material *wMat, G4LogicalVolume *wLV, G4double sPosZ, G4double sDepth) {
    nist = nistMan;
    worldMat = wMat;
    worldLV = wLV;

    detContainerHalfSize = G4ThreeVector(2.5 * cm, 40 * cm, 2.5 * cm);
    detContainerPos = G4ThreeVector(0, 0, sPosZ + sDepth / 2 + detectorPosZ + detContainerHalfSize.z());

    detectorType = "NMThermal";

    init();
}

void NMThermal::Construct() {
